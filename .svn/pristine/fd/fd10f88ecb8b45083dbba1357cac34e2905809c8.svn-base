...
public class UserDAOImpl implements UserDAO{
    ...
    private static final String SQL_SELECT_UNAFFECTED_STUDENTS = 
            "SELECT users.id,users.first_name,users.last_name,users.email,users.phone,users.password,users.userable_id ,users.userable_type FROM users " +
            "WHERE users.userable_id IN(" +
                    "SELECT id FROM students WHERE class_id IS NULL" +
            ") AND users.userable_type = \"students\" ORDER BY last_name " ;
    private static final String SQL_SELECT_STUDENTS_IN_CLASS =
            "SELECT * FROM users WHERE userable_id IN (" +
                    "SELECT id FROM students WHERE class_id = ?" +
            ") AND userable_type = 'students' ORDER BY last_name ";
