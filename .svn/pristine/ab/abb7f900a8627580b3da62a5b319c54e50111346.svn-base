...
public class RegistrationForm extends Form {
    ...
    public RegistrationForm(UserDAO userDAO, StudentDAO studentDAO, ProfessorDAO professorDAO){
        this(userDAO);
        this.studentDAO = studentDAO;
        this.professorDAO = professorDAO;
    }

    public RegistrationForm(UserDAO userDAO, StudentDAO studentDAO, ProfessorDAO professorDAO, DepartmentDAO departmentDAO){
        this(userDAO);
        this.studentDAO = studentDAO;
        this.professorDAO = professorDAO;
    ...
    public User register(Request request) {
        ...
        String phone = request.getFieldValue(PHONE_FIELD);
    ...
    private void validatePhone( String phone, User user ) {
        try {
    ...
    private void phoneValidation( String phone ) throws FormValidationException {
        if ( phone != null ) {
            if ( !phone.matches( "(\\d+)" ) ) {
                throw new FormValidationException( "Numéro de téléphone invalide" );
            } else if ( userDAO.findByPhone( Long.parseLong(phone)) != null ) {
                throw new FormValidationException( "Numéro de téléphone déjà  utilisée" );
            }
        } else {
            throw new FormValidationException( "Numéro de téléphone non saisie" );
        }
    }
